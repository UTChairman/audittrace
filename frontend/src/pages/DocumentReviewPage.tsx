...
import { getDocument, getExtraction, getOcr, listFindings } from "../api/client";
...
export function DocumentReviewPage() {
  ...
  const duplicateFinding = findings.find(
    (finding) =>
      finding.check_type === "duplicate_document" &&
      (finding.document_id === id ||
        finding.related_document_id === id ||
        finding.field_citations.some((citation) => citation.document_id === id))
  );
  ...
  return (
    <div className="space-y-4">
      ...
      {document.duplicate_of_document_id && (
        <p className="rounded-md border border-medium/30 bg-medium/10 px-3 py-2 text-sm">
          Exact copy of{" "}
          <Link
            className="font-medium text-forest underline-offset-2 hover:underline"
            to={`/documents/${document.duplicate_of_document_id}`}
          >
            document {document.duplicate_of_document_id}
          </Link>
          . Uploading the same file twice may indicate a double payment risk.
          {duplicateFinding && (
            <>
              {" "}
              <Link
                className="font-medium text-forest underline-offset-2 hover:underline"
                to={`/findings/${duplicateFinding.id}`}
              >
                Open duplicate document finding
              </Link>
              .
            </>
          )}
