...
def test_list_ocr_complete_document_ids_can_include_failed() -> None:
    engine = create_engine("sqlite:///:memory:")
    Base.metadata.create_all(engine)
    db: Session = sessionmaker(bind=engine)()
    for filename, status in (
        ("old.pdf", "ocr_complete"),
        ("failed.pdf", "extraction_failed"),
        ("done.pdf", "extracted"),
        ("pending.pdf", "pending"),
    ):
        db.add(
    ...
    db.commit()
    assert list_ocr_complete_document_ids(db) == [1]
    assert list_ocr_complete_document_ids(db, include_failed=True) == [1, 2]
